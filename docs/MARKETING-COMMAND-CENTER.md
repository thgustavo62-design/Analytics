...
- ✅ Tela: nova aba **Precificação** em Marketing — **seletor de grupo** (escolho o grupo → todos os produtos que valem promoção nele, preço a colocar + margem), ranking geral, busca por produto; qualquer linha abre o detalhe (curva SVG lucro×desconto, recomendado + break-even, 3 preços para testar, comparação com a promoção planejada). O `.detalhe` de cada produto vem **embutido no snapshot** → funciona no site publicado (sem backend).
- ✅ **Interligação:** `promo-pricing.precoRapido()` alimenta o **Command Center** (`anunciar[].promo` = preço a colocar / margem / lucro) e a tela de **Concorrentes** (`onde_reagir[].reagir_com` = preço para reagir + se cobre o concorrente). Default de duração de promo: 7 dias (`config/elasticidade.json`).
- ✅ Testes: `test/promo-pricing.test.js` (+8 → 118).
