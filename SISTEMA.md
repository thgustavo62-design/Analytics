...
| `social-analise.js` | `analiseSocial(loja)` — métricas orgânicas mês a mês (série + variação, delta do print ou calculado), tráfego pago (investimento / CPC / CPM / CTR / custo por resultado por mês) e **cruzamento com o faturamento** da loja (co-movimento, nunca atribui venda ao anúncio). Determinístico. |
...
| `marketing/command-center.js` | **Fase A** — `commandCenter(loja)`: plano do dia de uma loja — `anunciar[]` (ranqueado, com papel + ação + 3 motivos com evidência + sub-scores + **`promo`**: preço a colocar / desconto / margem na promo / lucro incremental, via `promo-pricing.precoRapido`), `nao_anunciar[]` (motivo curto + motivos + substituto), `alertas[]` (ruptura com venda relevante, categoria sob ataque, capital parado, feed faltando) |
...
| `marketing/promo-pricing.js` | **Precificação de promoção** — `precificarProduto(loja, {ean\|produto_id\|descricao, duracaoDias})`: varre descontos 0..teto, projeta unidades pela **elasticidade-preço da categoria em contexto de promoção** (`config/elasticidade.json`, premissa enquanto não há histórico), devolve `recomendado` (maximiza o **lucro incremental do próprio SKU** — o efeito-cesta é exibido em `efeito_cesta_estimado` mas **não decide**), `testar` (conservador / recomendado / agressivo), `curva` lucro×desconto, `limites` (break-even, desconto máx. sem prejuízo, teto pelo piso de margem) e comparação com a `promocao_planejada` da tabela. `oportunidadesPromo(loja, {n, duracaoDias})`: "**o que colocar em promoção**" — ranqueia os candidatos A/B (que movem ≥1 unidade extra) pelo lucro incremental da melhor promoção de cada um (promo de 7 d por padrão); **`por_grupo[]`** = para CADA categoria, todos os produtos que valem promoção com preço a colocar + margem; cada linha (ranking e grupo) já traz `.detalhe` embutido (o site publicado não tem backend para recalcular por produto). Sem custo (próprio ou proxy) → balde `sem_custo`. `precoRapido(loja, opts, produtos)` → `Map(produto_id → preço/desc/margem/lucro)` usado pelo Command Center e pela tela de Concorrentes. Custo pode ser proxy da outra loja (marcado). Todo resultado leva o aviso da premissa. |
...
| `parsers/social-vision.js` | **lê prints de rede social** (Instagram/Meta) — visão do `claude-sonnet-5` (`SOCIAL_VISION_MODEL`). Só transcreve o que está na imagem; sem a chave, a ingestão de imagem lança um erro claro. |
...
`POST /upload/print` (multipart `arquivo` = imagem + `loja`; lê o print por visão — precisa `ANTHROPIC_API_KEY`). O caminho da inbox também aceita `.png/.jpg/.webp` (nome do arquivo diz a loja).
...
| **Redes Sociais** | métricas orgânicas mês a mês (card por métrica com valor, variação e sparkline) · leitura automática · **Tráfego pago** (tabela mensal: investido / cliques / resultados / CPC / CPM / CTR / custo por resultado + leitura) · **Cruzamento com o faturamento** da loja (co-movimento, com ressalva de causa) · **Prints lidos** (auditoria do que a IA transcreveu) · botão **🖼️ Ler um print** (`/upload/print`). Sem `ANTHROPIC_API_KEY` a tela avisa e aponta o formulário manual. |
...
| `elasticidade.json` | precificação de promoção: elasticidade-preço **em contexto de promoção** por categoria (% de unidades por 1% de desconto), `default`, `uplift_max`, `passo_desconto`, `desconto_teto`, `piso_margem_pct`, `duracao_dias_padrao` (7 — duração da promo projetada no ranking e no detalhe), `halo_r$_por_unidade` (efeito-cesta exibido, não entra na recomendação). Premissa até haver histórico de promoções para calibrar. |
...
`npm test` (`node --test`) — **124 testes**, arquivos em separado (`process.env.VA_DB_PATH`
